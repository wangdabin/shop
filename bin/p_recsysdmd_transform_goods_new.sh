...
#*来源表 --%@SOURCE_TABLE:recsysdmd.goods
#*目标表 --%@TARGET_TABLE:recsys02.goods
#*执行周期 人工调用
####################################################################
THIS="$0"
...
id char(36) primary key COMMENT '商品标识',
platform_type int COMMENT '平台类型编号',
goods_code  char(36) COMMENT '商品号码',
product_type int COMMENT '产品类型编号',
business text COMMENT '业务部门',
...
#分析
#echo "开始分析数据"
$SPARK_HOME/spark-submit --master $SPARK_MASTER_URL --executor-memory 5G --total-executor-cores 24 --class sql.GoodsExportMysqlNew $JAR_PATH
