...
//    val myRatings = loadRatings("/Users/wangdabin1216/IdeaProjects/sbtSpark/ml-1m/personalRatings.txt")
//        val myRatingsRDD = sc.parallelize(myRatings, 1)

    //样本数据目录
    val movielensHomeDir = "/Users/wangdabin1216/work/lenovo/sbtSpark/ml-1m"

    //装载样本评分数据，其中最后一列Timestamp取除10的余数作为key，Rating为值，即(Int，Rating)
    val ratings = sc.textFile(movielensHomeDir + "/rating.dat").map {
      line =>
        val fields = line.split("\t")
        // format: (timestamp % 10, Rating(userId, movieId, rating))
        ((Math.random() * 10 +1).toInt, Rating(fields(0).toInt, fields(1).toInt, fields(2).toDouble))
    }
...
//    val myRatedMovieIds = myRatings.map(_.product).toSet
    ...
    val arr = Array(1,2,36,41,58,6)
    val users = sc.parallelize(arr)

    val allR = users.map(("A",_)).join(candidates.map(("A",_))).map(_._2)
    ...
    val recommendations = bestModel.get
      ...
      .groupBy(_.user)
      .flatMap(x =>{
      x._2.toList.sortBy(-_.rating).take(10)
    }).map(x =>{
